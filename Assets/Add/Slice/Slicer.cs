...
class Slicer
{
    public static GameObject[] Slice(Plane plane, GameObject objectToCut)
    {
        ...
        #region New UV
        NewUV(positiveSideMeshData);
        NewUV(negativeSideMeshData);
        #endregion
    ...
    private static void NewUV(Mesh positiveSideMeshData)
    {
        Vector3[] verteces = positiveSideMeshData.vertices;
        Vector2[] newUV = new Vector2[verteces.Length];

        for (int i = 0; i < newUV.Length; i++)
        {
            newUV[i].x = -verteces[i].x -0.5f;
            newUV[i].y = -verteces[i].z -0.5f;
        }

        positiveSideMeshData.uv = newUV;
    }
